# Selendra Blockchain Explorer - TODO List

## Project Goal

To build a comprehensive blockchain explorer for Selendra—an EVM-compatible, L1 Substrate-based network. This explorer will provide unified, first-class support for both EVM and Wasm smart contracts, enabling users to explore blocks, transactions, accounts, contracts, tokens, and validators. Key features include wallet integration for asset management and contract interaction, a staking dashboard, contract management (saving favorites), and code snippet storage. The ultimate aim is to deliver a robust, maintainable, and scalable solution that empowers users to explore and interact with the Selendra blockchain, tailored to the Cambodian ecosystem.
...
- [ ] **API Route Placeholders:**
    - [x] API route structure defined. (As per `implementation-plan.md`)
    - [ ] Refine API route definitions based on finalized frontend needs.

**Frontend Infrastructure:**
- [x] **Basic Page Structure & Routing:** (As per `implementation-plan.md`)
- [x] **Context Providers (API & Wallet):** (As per `implementation-plan.md`)
- [x] **Component Structure (Layout & Pages):** (As per `implementation-plan.md`)
...
**Note:** This TODO list is comprehensive and assumes all features from the `implementation-plan.md` are desired. Adjust priorities and scope based on project timelines and resources. 