	...
	return (
		...
		>
			<Table.Column
				...
				render={(stash_address, record) => {
					if (record.name === "") {
						return (
							<Link
								to={`/validator/${stash_address}?block_id=${record.block_height}`}
			...
			<Table.Column
				...
				render={(total_stake) => <p>{standardBalance(total_stake)} SEL</p>}
			/>
			<Table.Column
				title="Self Stake"
				dataIndex="self_stake"
				render={(self_stake) => <p>{standardBalance(self_stake)} SEL</p>}
			/>
			<Table.Column
				title="Nominator"
