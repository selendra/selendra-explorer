...
export const generateMockToken = (
  ...
  return {
    ...
    symbol,
    decimals: type === 'erc20' ? (symbol === 'USDT' || symbol === 'USDC' ? 6 : 18) : 0,
    totalSupply: type === 'erc20' 
...
export const getMockTokens = (page: number, pageSize: number, type?: TokenType, networkType?: NetworkType) => {
  ...
  if (networkType) {
    filteredTokens = filteredTokens.filter(token => token.networkType === networkType);
  }
