				...
				<Card className="block-detail-card">
					...
					<table className="table">
						<tbody>
							...
							<tr className="tr-style">
								<td>Hash</td>
								<td>{block_by_pk?.block_hash}</td>
								<CopyOutlined
									style={{ fontSize: "20px", marginTop: "16px" }}
									onClick={() =>
										navigator.clipboard
											.writeText(block_by_pk?.block_hash)
											.then(() =>
							...
							<tr className="tr-style">
								<td>Block Author</td>
								<td>{block_by_pk?.block_author}</td>
								<CopyOutlined
									style={{ fontSize: "20px", marginTop: "16px" }}
									onClick={() =>
										navigator.clipboard
											.writeText(block_by_pk?.block_author)
											.then(() =>
												notification.success({
													message: "Copied",
												}),
