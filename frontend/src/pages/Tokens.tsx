...
const NetworkTypeButtons = ({ selected, onChange }: { 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-300 max-w-2xl leading-relaxed">
            Browse all tokens on the Selendra blockchain. Includes ERC20, ERC721 (NFT), ERC1155 tokens and native Substrate assets.
          </p>
        </div>
      ...
      <div className="mb-8 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
        <div className="space-y-5">
          ...
          <div>
            ...
            <NetworkTypeButtons selected={networkType} onChange={setNetworkType} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 hover:shadow-md transition-all duration-300">
          <div className="text-sm text-gray-500 dark:text-gray-400 mb-2 flex items-center">
            ...
            {tokenType === 'erc20' ? 'Total ERC20' : 
             tokenType === 'erc721' ? 'Total NFTs' : 
             tokenType === 'erc1155' ? 'Total ERC1155' : 'ERC20 Tokens'}
          </div>
          <div className="text-xl font-bold text-gray-900 dark:text-white">
            {!isLoading && data?.totalCount ? (
              tokenType ? data.totalCount.toLocaleString() : Math.floor(data.totalCount * 0.7).toLocaleString()
            ) : (
              <div className="h-7 bg-gray-200 dark:bg-gray-700 rounded-md w-24 animate-pulse"></div>
            )}
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-all duration-300">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
            ...
            {networkType && <span className="ml-2 text-sm text-gray-500 dark:text-gray-400">({networkType.toUpperCase()})</span>}
          </h2>
          
          {!isLoading && data && (
            <div className="text-sm text-gray-500 dark:text-gray-400 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-full">
              Showing tokens {((page - 1) * pageSize) + 1} to {Math.min(page * pageSize, data.totalCount)} of {data.totalCount.toLocaleString()}
            </div>
          )}
        </div>
          ...
          columns={[
            {
              ...
              accessor: (token) => (
                <div className="flex items-center group">
                  {token.logoUrl ? (
                    <div className="w-8 h-8 flex-shrink-0 mr-3 rounded-full overflow-hidden flex items-center justify-center bg-gray-100 dark:bg-gray-700">
                      <img
                        src={token.logoUrl}
                        alt={`${token.name} logo`}
                  ...
                  <div>
                    <Link to={`/tokens/${token.address}`} className="text-primary-600 dark:text-primary-400 hover:underline font-medium">
              ...
              accessor: (token) => (
                <div className="flex items-center space-x-2">
                  <span className={`px-2 py-1 rounded-md text-xs ${
                    token.tokenType === 'erc20' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' :
                    token.tokenType === 'erc721' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300' :
                    token.tokenType === 'erc1155' ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300' :
            ...
            },
            {
              ...
              accessor: (token) => (
                <div className="flex flex-col">
                  <TimeAgo timestamp={token.createdAt} className="text-gray-900 dark:text-white" />
            ...
            },
            {
              header: '',
              accessor: (token) => (
                <Link to={`/tokens/${token.address}`} className="flex items-center justify-center text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
