...
/**
 * Transactions page component that displays a list of all transactions
 * Supports filtering by address via query parameters
 */
const Transactions: React.FC = () => {
  // Get address from URL query params if available
  ...
  // Copy address to clipboard utility function
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-300 max-w-2xl leading-relaxed">
            {address
              ? `Transactions for address ${address.substring(
                  0,
                  8
          ...
          <div className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
            {!isLoading && data?.items[0] ? (
              <div className="flex items-center">
                <Link
                  to={`/transactions/${data.items[0].hash}`}
                  className="font-mono truncate"
                >
                  {`${data.items[0].hash.substring(
                    0,
                    10
                  )}...${data.items[0].hash.substring(
                    data.items[0].hash.length - 8
                  )}`}
                </Link>
                <button
        ...
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          {!isLoading && data && (
            <div className="text-sm text-gray-500 dark:text-gray-400 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-full">
              Showing transactions {(page - 1) * pageSize + 1} to{" "}
              ...
              accessor: (tx) => (
                <div className="flex items-center group">
                  <Link
                    ...
                    className="text-primary-600 dark:text-primary-400 hover:underline font-mono text-sm truncate max-w-[120px] sm:max-w-[160px]"
                  >
            ...
            },
            {
              ...
              accessor: (tx) => (
                <Link
                  ...
                  className="text-primary-600 dark:text-primary-400 hover:underline font-medium group flex items-center"
                >
