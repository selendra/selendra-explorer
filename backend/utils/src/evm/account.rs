...
use model::{account::EvmAccountInfo, contract::{ContractType, EvmContractTypeInfo}};
...
impl AccountQuery {
    ...
    pub async fn query_account(&self, address: &str) -> Result<EvmAccountInfo, ServiceError> {
        let addr: Address = address
            .parse()
            .map_err(|e| ServiceError::InvalidTransactionData(format!("Invalid address {}: {}", address, e)))?;
    ...
    async fn detect_contract_type(&self, address: Address, code: &Bytes) -> Result<EvmContractTypeInfo, ServiceError> {
    ...
    async fn detect_by_erc165(&self, address: Address) -> Result<ContractType, ServiceError> {
        ...
        // Check ERC721
        if let Ok(supports_erc721) = self.call_supports_interface(address, &erc721_interface_id).await {
            if supports_erc721 {
        ...
        // Check ERC1155
        if let Ok(supports_erc1155) = self.call_supports_interface(address, &erc1155_interface_id).await {
            if supports_erc1155 {
    ...
    async fn call_supports_interface(&self, address: Address, interface_id: &[u8; 4]) -> Result<bool, ServiceError> {
    ...
    async fn detect_by_function_signatures(&self, address: Address, code: &Bytes) -> Result<ContractType, ServiceError> {
    ...
    fn count_signature_matches(&self, code: &Bytes, signatures: &[&str]) -> usize {
        ...
        for sig in signatures {
            ...
            if code.as_ref().windows(4).any(|window| window == selector_bytes) {
                matches += 1;
            }
        }
    ...
    async fn create_contract_info(&self, contract_type: ContractType, address: Address) -> Result<EvmContractTypeInfo, ServiceError> {
    ...
    async fn get_erc20_metadata(&self, address: Address) -> Result<TokenMetadata, ServiceError> {
        let name = self.call_string_function(address, &[0x06, 0xfd, 0xde, 0x03]).await.ok(); // name()
        let symbol = self.call_string_function(address, &[0x95, 0xd8, 0x9b, 0x41]).await.ok(); // symbol()
    ...
    async fn call_string_function(&self, address: Address, selector: &[u8; 4]) -> Result<String, ServiceError> {
        let call_request = TransactionRequest::new()
            .to(address)
            .data(Bytes::from(selector.to_vec()));
        ...
        if result.len() < 64 {
            return Err(ServiceError::InvalidTransactionData("Invalid response length".to_string()));
        }
        ...
        if result.len() < 64 + length {
            return Err(ServiceError::InvalidTransactionData("String length exceeds data".to_string()));
        }
    ...
    async fn call_decimals_function(&self, address: Address) -> Result<u8, ServiceError> {
        ...
        if result.len() != 32 {
            return Err(ServiceError::InvalidTransactionData("Invalid decimals response".to_string()));
        }
    ...
    async fn call_total_supply_function(&self, address: Address) -> Result<U256, ServiceError> {
        ...
        if result.len() != 32 {
            return Err(ServiceError::InvalidTransactionData("Invalid totalSupply response".to_string()));
        }
