...
impl ExtrinsicInfo {
    pub fn new(block: Block<Header<u32, BlakeTwo256>, OpaqueExtrinsic>) -> Self {
        Self { block }
    }
    ...
    async fn decode_extrinsic_details(
        ...
        if length_bytes >= raw_data.len() || total_length > raw_data.len() {
            return Err(ServiceError::SubstrateError(format!(
                "Invalid extrinsic length: {} bytes, data length: {}",
                total_length,
                raw_data.len()
            )));
        }
    ...
    fn decode_signature_info_v2(
        ...
        let signer_bytes = raw_data
            .get(cursor..cursor.saturating_add(32))
            .ok_or_else(|| {
                ServiceError::SubstrateError("Insufficient data for signer".to_string())
            })?;
        let signer = format!("0x{}", hex::encode(signer_bytes));
        cursor = cursor.saturating_add(32);

        let signature_bytes = raw_data
            .get(cursor..cursor.saturating_add(64))
            .ok_or_else(|| {
                ServiceError::SubstrateError("Insufficient data for signature".to_string())
            })?;
    ...
    fn calculate_call_start_signed(
        ...
        if cursor < raw_data.len() {
            match self.decode_compact_length(&raw_data[cursor..]) {
                ...
                Err(e) => {
                    return Err(ServiceError::SubstrateError(format!(
                        "Failed to decode nonce in call start calculation: {}",
                        e
                    )));
                }
            }
        ...
        if cursor < raw_data.len() {
            match self.decode_compact_length(&raw_data[cursor..]) {
                ...
                Err(e) => {
                    return Err(ServiceError::SubstrateError(format!(
                        "Failed to decode tip in call start calculation: {}",
                        e
                    )));
                }
            }
    ...
    fn extract_call_args(
        ...
        match (pallet_index, call_index) {
            // Timestamp::set
            (3, 0) if call_args.len() >= 4 => match self.decode_compact_length(call_args) {
                        ...
                        Ok((input_len, len_bytes)) => {
                            ...
                            if cursor + input_len <= call_args.len() && input_len > 0 {
                                if let Some(input_bytes) = call_args.get(cursor..cursor + input_len)
                                {
                                    args.push((
                                        "input".to_string(),
                                        format!("0x{}", hex::encode(input_bytes)),
                                    ));
                                    cursor += input_len;
                                }
                            } else {
                        ...
                        Ok((init_len, len_bytes)) => {
                            ...
                            if cursor + init_len <= call_args.len() && init_len > 0 {
                                if let Some(init_bytes) = call_args.get(cursor..cursor + init_len) {
                                    args.push((
                                        "init".to_string(),
                                        format!("0x{}", hex::encode(init_bytes)),
                                    ));
                                    cursor += init_len;
                                }
                            } else {
