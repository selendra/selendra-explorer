...
async fn handle_client_message(
    ...
    match msg {
        WebSocketMessage::Subscribe { topic } => {
            ...
            let valid_topics = ["blocks", "transactions", "substrate_blocks", "substrate_events"];
            if !valid_topics.contains(&topic.as_str()) {
                let error_response = WebSocketMessage::Error {
                    message: format!("Invalid topic: {}. Valid topics are: {}", topic, valid_topics.join(", ")),
                };
...
async fn send_transaction_update(
    ...
    match transaction_service.get_latest().await {
        Ok(Some(transaction)) => {
            ...
            let message = WebSocketMessage::TransactionUpdate { data: transaction_data };
            let message_text = serde_json::to_string(&message)?;
